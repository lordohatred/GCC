...
public class MainMenuScript : MonoBehaviour {
	...
	void Start(int windowID){
		if (GUI.Button (new Rect (25, 25, Screen.width / 4.45f, 50), "Start Game"))
			StartGame ();
        if (GUI.Button (new Rect (25, 85, Screen.width / 4.45f, 50), "Start Multiplayer")) {
            StartMultiplayer ();
        }
		if (GUI.Button (new Rect (25, 145, Screen.width / 4.45f, 50), "Change Map")) {
			showStart = false;
			showMap = true;
		}
		if (GUI.Button (new Rect (25, 205, Screen.width / 4.45f, 50), "Game Options")) {
			showStart = false;
			showDiff = true;
		}
