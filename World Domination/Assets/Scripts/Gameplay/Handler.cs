...
public class Handler : MonoBehaviour {
	...
	void FixedUpdate () {
		if (canSpawn){
            ...
            if (player == null && Application.loadedLevel == 1)
                player = GameObject.FindGameObjectWithTag ("Country");
            if (player != null && Application.loadedLevel == 1) {
    ...
    void InstantiateCountries () {
        if (maxCountries != 0) {
            if (spawnedCountries < maxCountries) {
                Instantiate (Country[Random.Range (0, Country.Length)],
                    new Vector3 (Random.Range (size.x, -size.x), Random.Range (size.y, -size.y), 0),
                    transform.rotation);
            }
        } else {
            Instantiate (Country[Random.Range (0, Country.Length)],
    ...
    void InstantiatePopulation () {
        Instantiate (pop,
             new Vector3 (Random.Range (size.x, -size.x), 
             Random.Range (size.y, -size.y), 0), transform.rotation);
	}
